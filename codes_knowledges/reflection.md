...
Из того, что было изучено, можно сделать следующее:
...
С `Override` - перезаписывание родительского метода.
Без него - полиморфизм переменной типа `Cat`, откуда и вызвался метод.
